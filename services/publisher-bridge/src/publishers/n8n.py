...
class N8nPublisher(BasePublisher):
    ...
    async def publish(self, request: PublishRequest) -> PublishResult:
        ...
        endpoint_path = str(request.metadata.get("endpoint_path", "/api/v1/workflows"))
        ...
        if not workflow_id:
            raise ValueError("n8n response did not include a workflow identifier")
        ...
        return PublishResult(
            ...
            success=True,
            status="published",
            message="n8n workflow published successfully.",
            external_refs={"n8n_workflow_id": workflow_id},
            artifacts=[
                PublishedArtifact(
                    kind="workflow_draft",
                    uri=f"n8n://workflows/{workflow_id}",
